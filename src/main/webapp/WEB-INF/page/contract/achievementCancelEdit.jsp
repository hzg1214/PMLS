...
<body>
	<h4>
		<label>业绩撤销变更</label> <span> <a href="javascript:void(0)"
			...
			<c:forEach items="${storeList}" var="store">
				<tr>
					<td><input type="checkbox" id="isCancel" 
						value="${store.storeId}"
						<c:if test="${store.isCancel eq '17202' || store.isCancel eq '17203'}"> name="Canceled" disabled="disabled"</c:if>
						...
						<c:if test="${store.isCancel eq '17201' || empty store.isCancel||store.approveState eq '0'||store.approveState eq '1'||store.approveState eq '2'||store.approveState eq '3'}"> name="isCancel" onclick="AchievementCancel.selectThis(this);" </c:if>>
					</td>
					<td>${store.storeNo}</td>
