...
function initCityList(orgCode) {
    ...
    ajaxGet(url, params, function (data) {
        ...
        if (list != null && list.length > 0) {
            var html = '';
            for (var i = 0; i < list.length; i++) {
                html = html + '<li class="multi-select-item"> <label><input type="checkbox" value="' + list[i].cityId + '" data-text="' + list[i].cityName + '"><span>' + list[i].cityName + '</span></label> </li>';
            }
...
$('#city').find('.multi-select').multiSelect({
    check: function ($instance) {
        ...
        var cityId = $instance.value.toString();
        if (isBlank(cityId)){
            resetCenter();
            return;
        }
...
function initCenterList(orgCode) {
    ...
    ajaxGet(url, {cityId: cityIdStr.replaceAll(",", "','"),orgCode: orgCode}, function (data) {
        var list = data.returnValue;
        if (list != null && list.length > 0) {
            var html = '';
            for (var i = 0; i < list.length; i++) {
