...
var tagArr = [  'ChangeSupplement','NewBusinessLicense', 'InformationPublicity','ConfirmationSheet','StorePhotos','Others',
				'TransferRights','NewSigning','InformationPublicity','OthersB'
				];

var picArr = [ 'biangengbuchong.PNG','xinyingyezhizhao.PNG', 'wangzhanjietu.PNG', 'fangyouqueren.PNG', 'store.png','qita.png',
				'quanliyiwu.PNG', 'xinqianhezuo.PNG' ,'wangzhanjietu.PNG', 'qita.png'];
 ...
 * @returns {Boolean}
 */
function Save(contractId,contractStatus) {
	...
	if(checkdata())
	{
		...
		var url = BASE_PATH + '/stopcontract/Save';
...
function defaultInfo(){
	...
	{ 
		...
		{//签约主体变更
	    	 ...
	    	 $("#signEnclosure").hide();
	    	 $("#mainchange").attr("checked",true);
	    	 $("#changeMain").show();
		}
		
		if (strs[i]== "20002")
		{//公司经营范围变更更
			$("#companyrange").attr("checked",true);
			$("#changeMain").hide();
		}
		
		if (strs[i]== "20003")
		{//公司注册地址变更
			$("#companychange").attr("checked",true);
