...
/** **************************demo js*************************** */
SceneInCommissionDetail = function() {
};
...
initList = function() {
	...
	httpGet('SceneInCommissionDetailListForm', 'LoadCxt', BASE_PATH + '/sceneInCommission/qSceneInCommissionDetail/q', function() {
		...
		pageInfo("SceneInCommissionDetailListForm", function() {
			initList();
		});
 ...
 * 
 */
SceneInCommissionDetail.search = function() {
	$('#curPage').val('1');
	initList();
};
 ...
 * 
 */
SceneInCommissionDetail.modify = function() {
	var type = 0;
	if (isItemSelected(type))
	{
		Dialog.confirm("确认要修改佣金吗？",
				function() {
					var url = BASE_PATH + "/sceneInCommission/qSceneInCommissionDetail/modify";
 ...
 * 
 */
SceneInCommissionDetail.confirm = function() {
	var type = 1;
	if (isItemSelected(type))
	{
		Dialog.confirm("确认要结算吗？",
				function() {
					var url = BASE_PATH + "/sceneInCommission/qSceneInCommissionDetail/confirm";
					var params = {reports: getReports(type)};
					submit(url, params);
				});
 ...
 * 
 */
SceneInCommissionDetail.modify2 = function(idx) {
	var type = 0;
 ...
 * 
 */
SceneInCommissionDetail.confirm2 = function(idx) {
	var type = 1;
	item = $('.J_estateItem')[idx];
		Dialog.confirm("确认要结算吗？",
				function() {
