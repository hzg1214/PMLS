...
initList = function() {
	...
	httpGetWithParams('storeStopCancelForm', 'LoadCxt', params, BASE_PATH + '/StoreStopCancel/list', function () {

		pageInfo("storeStopCancelForm", function () {
			initList();
		});
...
BizStop.view = function (id) {
	if(BizStop.dialog!=null && !BizStop.dialog.closed){
		BizStop.dialog.close();
	}
	var url = LOC_RES_BASE_PATH + '/StoreStopCancel/view/' + id;
	var title = '';
	var params = {};
	var dialogOptions = {
		zIndex : 1000
...
BizStop.reset=function (pageType) {
	$("#storeStopCancelForm :text").val("");
	$("input[type='checkbox']").prop("checked", false);
	$("#storeStopCancelForm").find("select").val("");
 ...
 * 审核通过
 */
BizStopAud.auditPass = function(id,storeId,storeNo,storeName,userId,addressDetail) {
	Dialog.confirm("确认审核通过吗？", function() {
		var url=BASE_PATH + '/StoreStopCancel/auditPass';
		var paramsData={"id":id,"storeId":storeId,"storeNo":storeNo,"storeName":storeName,"userId":userId,"addressDetail":addressDetail};
		ajaxGet(url,paramsData,function(data) {
				if(data && data.returnCode == '200'){
					location.href= BASE_PATH +"/StoreStopCancel";
 ...
 * 审核退回
 */
BizStopAud.auditReturn=function(id,storeId,storeNo,storeName,userId,addressDetail) {
	var url=BASE_PATH + '/StoreStopCancel/toRejectStopCancel';
	var params={"id":id,"storeId":storeId,"storeNo":storeNo,"storeName":storeName,"userId":userId,"addressDetail":addressDetail};
	var dialogOptions = {
		width : 300,
		height : 100,
		ok : function() {
			// 确定
			var reback = BizStopAud.saveAudit(id,storeId);
			return reback;
		},
		okVal : '确定',
		cancel : true,
...
//提交驳回原因并更改审核状态
BizStopAud.saveAudit = function(){
	if (Validator.validForm("rejectStopForm")) {
		var param = $("#rejectStopForm").serialize();
		var url=BASE_PATH + '/StoreStopCancel/rejectStopCancel';
