...
/*            {field: 'dealAmount',title: '成销总价',width:150, align: 'center',rowspan: 2
    ...
    active = {
        ...
        export: function () {
            if (valid()) {
                ...
                var url = BASE_PATH + '/lnkYjQtFy/exportCheck?cityNo=' + $("#cityNo").val() + "&projectDepartmentId=" + $("#projectDepartmentId").val()
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.returnCode == 200 || data.returnCode == '200') {
                            window.location.href =  BASE_PATH + '/lnkYjQtFy/export?cityNo=' + $("#cityNo").val() + "&projectDepartmentId=" + $("#projectDepartmentId").val()
                                + "&projectNo=" + trimStr($("#projectNo").val()) + "&reportNo=" + trimStr($("#reportNo").val()) + "&countDateStart=" +$("#countDateStart").val()
                                + "&countDateEnd=" + $("#countDateEnd").val() + "&estateType=" + $("#estateType").val()
                                + "&estateNmKey=" + getXMSelectIds(formSelects.value('estateNmKey'));
        ...
        import: function () {
            ...
            layer.open({
                ...
                title: 'EXCEL导入',
                area: ['500px', '400px'],
                content: BASE_PATH + '/lnkYjQtFy/toView'
            });
    ...
    function reloadData(optionsData) {
        ...
        table.reload('contentReload', {
            url: BASE_PATH + '/lnkYjQtFy/getLnkYjQtFyList',
            cols: setCols(),
            where: optionsData,
            page:{
                curr: 1 //重新从第 1 页开始
