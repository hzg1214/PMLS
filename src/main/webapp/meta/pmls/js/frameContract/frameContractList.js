...
layui.use(['element', 'laydate', 'form', 'table', 'layer'], function () {
    ...
    function setCols() {
        ...
        row1.push(
            ...
            {field: 'companyNo',title: '渠道编号',width:100, align: 'center'},
            {field: 'companyName',title: '渠道公司',width:300, align: 'left'},
            ...
            {title: '操作', width:200,align: 'left',fixed:'right',templet: function(row) {
                    ...
                    if(((row.approveState=='10401' || row.approveState=='10404') && (row.submitOAStatus == '21201' || row.submitOAStatus == '21204'))){
                        ...
                        showContent+='<a class="layui-btn layui-btn-danger layui-btn-mini" onclick="cancel(' + row.id + ')">作废</a>';
                    }
    ...
    var active = {
        reload:function(){
            ...
            sessionStorage.removeItem('FRAME_CONTRACT_SEARCH');
            reloadData(optionsData);
        },
        addContract:function () {//新增合同
            var url=ctx+"/pmlsFrameContract/addFrameContractPage";
    ...
    function reloadData(optionsData){
        ...
        var sessionData=sessionStorage.getItem('FRAME_CONTRACT_SEARCH');
        var pageIndex=1;
        ...
        table.reload('contentReload', {
            ...
            page:{
                ...
                sessionStorage.setItem("FRAME_CONTRACT_SEARCH",JSON.stringify(optionsData));
            }
        });
    }
...
//修改
function update(id){
    var url=ctx+"/pmlsFrameContract/addFrameContractPage?id="+id;
    parent.redirectTo('append',url,'编辑联动框架协议');
}
//作废
function cancel(id){
    parent.layer.confirm("确认作废吗？",{icon: 3, title:'提示'},function(){
        parent.layer.load(2);
        $.ajax({
            url: ctx + '/pmlsFrameContract/cancel',
            type: 'post',
            data:{id:id},
            dataType: 'json',
