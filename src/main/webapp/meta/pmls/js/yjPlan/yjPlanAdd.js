...
function addCol(){
    ...
    contentTrStr += '<span name="companyShowName"></span><a class="layui-btn layui-btn-xs" style="float: right;margin-top: 5px;" onclick="showSelect(this)">选择</a>';
    contentTrStr += '</div></td>';
...
/*            if(!checkIsNumber(totalPercentage)){
        ...
        for(var i=0;i<yjPlanDetailList.length;i++){
            ...
            for (var j=0;j<yjPlanDetailList.length;j++) {
                ...
                var conditionEndNext = yjPlanDetailList[j].conditionEnd;
                if((i+1)==(j) && parseFloat(conditionBeginNext) != parseFloat(conditionEndPre) ){
                    parent.layer.alert("第"+(i+1)+"条件区间不合法", {icon: 2, title: '提示'});
                    checkFlag = false;
                    break;
                }
            }
