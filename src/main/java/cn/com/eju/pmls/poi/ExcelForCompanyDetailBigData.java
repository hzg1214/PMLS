 ...
 * 
 */
public class ExcelForCompanyDetailBigData extends ExcelUseXmlAbstract {
    private final LogHelper logger = LogHelper.getLogger(this.getClass());

    private RptCompanyDetailService rptCompanyDetailService = new RptCompanyDetailService();
    ...
    public void downloadSheet(Map<String, Object> reqMap, File file, Map<String, Object> titleMap) throws IOException {
        ...
        XSSFSheet sheet = wb.createSheet("经纪公司明细");
    ...
    private void createSheet(Writer out, Map<String, Object> reqMap,Map<String, Object> titleMap) throws Exception {
        ...
        logger.info("经纪公司明细 第1次查询开始。。。");
        ResultData<?> reback = rptCompanyDetailService.queryCompanyDetailList(reqMap, pageInfo);
        ...
        if (contentList != null && totalCount > contentList.size()) {
            ...
            for (int i = 2; i <= pageCount; i++) {
                ...
                logger.info("经纪公司明细 第" + i + "次查询开始,共" + pageCount + "次。");
                reback = rptCompanyDetailService.queryCompanyDetailList(reqMap, pageInfo);
                logger.info("经纪公司明细 第" + i + "次查询结束,共" + pageCount + "次。");
    ...
    private int createSheetHead(SpeedSheetWriter sw, int index, List<String> listMerge,Map<String, Object> reqMap,Map<String, Object> titleMap) throws IOException {
        ...
        nowColIndex = createBlock(sw, "公司编号", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "公司名称", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "业务类型", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "是否签框架", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "框架开始时间", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "框架结束时间", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "框架过审时间", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "框架协议创建人", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "旗下门店数", 1, 2, listMerge);
        ...
        nowColIndex = createBlock(sw, "是否签框架", 1, 0, listMerge);
        nowColIndex = createBlock(sw, "框架开始时间", 1, 0, listMerge);
        nowColIndex = createBlock(sw, "框架结束时间", 1, 0, listMerge);
        nowColIndex = createBlock(sw, "框架过审时间", 1, 0, listMerge);
        nowColIndex = createBlock(sw, "框架协议创建人", 1, 0, listMerge);
        nowColIndex = createBlock(sw, "旗下门店数", 1, 0, listMerge);
    ...
    private int createSheetBody(SpeedSheetWriter sw, int index, LinkedHashMap<String, Object> map
        ...
        if("yes".equals(map.get("isFrameContract"))) {
        	sw.createCell(i++, "是", bodyStyle);
        }else {
        	sw.createCell(i++, "否", bodyStyle);
        }
        setCreateDateCell("frameDateStart", i++, dateBlackStyle, map, sw);
        setCreateDateCell("frameDateEnd", i++, dateBlackStyle, map, sw);
        setCreateDateCell("frameApprovePassDate", i++, dateBlackStyle, map, sw);
        sw.createCell(i++, map.get("frameCreateUserName") == null ? "" : map.get("frameCreateUserName").toString(), bodyStyle);
        sw.createCell(i++, map.get("storeNums") == null ? "" : map.get("storeNums").toString(), bodyStyle);
