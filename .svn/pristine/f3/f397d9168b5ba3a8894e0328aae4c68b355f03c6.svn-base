...
layui.use(['element', 'laydate', 'form', 'table', 'layer','upload'], function () {
    ...
    var dateLifeStartObj = laydate.render({
        elem: '#dateLifeStart',
        min: cooperationInfo.htDateStart.substring(0,10),
        max: cooperationInfo.dateLifeEnd.substring(0,10),
        trigger: 'click',
    ...
    var dateLifeEndObj = laydate.render({
        elem: '#dateLifeEnd',
        min: cooperationInfo.dateLifeStart.substring(0,10),
        max: cooperationInfo.htDateEnd.substring(0,10),
        trigger: 'click',
        done: function (value, date, endDate) {
    ...
    //edit
    $("#contractNameType").val(cooperationInfo.contractNameType);
    if(cooperationInfo.contractNameType == '30005' || cooperationInfo.contractNameType==30005){
        $("#contractNameDescDiv").show();
    }
    ...
    if(cooperationInfo.signType=='29201'){

    }else if(cooperationInfo.signType=='29202'){
        $("input[name='htType']").prop("disabled","disabled");
        $("input[name='htCategory']").prop("disabled","disabled");
        $("#dyblDiv").hide();
        form.render();
    ...
    if(cooperationInfo.htType=='1671583364489561661'){
        $("#dyblDiv").hide();
    }else if(cooperationInfo.htType=='-1835969197405944893'){
        $("#dyblDiv").show();
    }
    form.render();
    ...
    //初始化加载图片
    loadImageList('uploadContractImg',uploadContractImgList,true);
    loadImageList('uploadOtherImg',uploadOtherImgList,true);
...
//            }
 ...
 /*           if((htDateEnd < dateLifeEnd) || (dateLifeStart < htDateStart)){
            ...
            $.ajax({
                url: BASE_PATH + '/cooperationController/updateCooperation',
                type: 'post',
                data:formData,
                dataType: 'json',
                success: function (data) {
