 ...
 *            验证性表单元素提示信息规则有如下两种: 1.同行显示,html事例:
 *            <tr>
 *            <th scope="row"><i>*</i><span for="${telephone}">手机号码：</span></th>
 *            <td><input type="text" name="telephone" id="telephone"
 *            notnull="true"/><span class="namejg marleft10"><b></b></span></td>
 *            </tr>
 *            必备条件:a.标签span必须存在属性for="验证文本框id";b.验证文本框后面必须存在span
 *            class="namejg";c.验证文本框和提示信息的span必须有一个共同的父级元素
 * 
 * 2.换行显示,html事例:
 * <tr>
 * <th scope="row" width="15%"><span for="${name}">名称:</span></th>
 * <td width="50%"><input type="text" name="fileName" id="name" notnull="true"
 * maxlength="20"/></td>
 * </tr>
 * <tr id="${name}Tip" style="display: none;">
 * <th scope="row">&nbsp;</th>
 * <td><span class="namejg"></span></td>
 * </tr>
 * 必备条件:a.标签span必须存在属性for="验证文本框id";b.提示信息行id必须是 验证文本框id+Tip,此行中必须要有span
 * class="namejg"的元素
 */
Validator.validElement = function(jqueryObj, needFocus) {
	...
	var warningSpan = jqueryObj.parent().find(".fc-warning");
	// 换行提示信息区块
	...
	if (warningSpan.length <= 0) {
		...
		label = '<span class="fc-warning"></span> ' + label;
	} else {
		label = '<span class="fc-warning"></span> ' + label;
	}
	// 验证前重置信息并隐藏
	...
	if (pwdCompare && $('#' + pwdCompare).val() != jqueryObj.val()) {
		warningBlock ? warningBlock.show() : $.noop();
		warningSpan.html(
				'<i class="icon-remove-sign i-icon-red"/></i> 两次确认密码不一致！')
				.show();
		needFocus ? jqueryObj.focus() : $.noop();
		return false;
	}
		...
		for (var i = 0; i < dataTypeArr.length; i++) {
			...
			if (dataType == 'posNumWithZero') {
				if (!isPosNumWithZero(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '请输入数字!').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
				// 整数除去0
			}
			if (dataType == 'posNumWithOutZero') {
				if (!isPosNumWithOutZero(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '必须为正整数!').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
				// 日期
				...
				if (!isAddress(jqueryObj.val())) {
					warningBlock?warningBlock.show():$.noop();
					warningSpan.html(label + '只能包含中文、字母、数字以及-,且不能出现连续两个-').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
			...
			}else if(dataType == 'moneyWithFlot'){
				if(!moneyWithFlot(jqueryObj.val())){
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '只能为数字格式！').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
			}else if (dataType == 'normal2') {//-除外
				if (!isNormal2(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '不能含非法字符!').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
			...
			}else if (dataType == 'normal3') {//-除外
				if (!isNormal3(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '不能含非法字符!').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
			...
			}else if (dataType == 'moneyWithTwo') {
				if (!moneyWithTwo(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '金额最多两位小数！').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
			}/*else if (dataType == 'bankAccount') {
				if (!isBankAccount(jqueryObj.val())) {
					warningBlock ? warningBlock.show() : $.noop();
					warningSpan.html(label + '银行账号请输入数字！').show();
					needFocus ? jqueryObj.focus() : $.noop();
					return false;
				}
			}*/
...
Validator.validDataType = function(jqueryObj, needFocus) {
...
Validator.validForm = function(formId) {
	var isValid = true;
	$(
			"form[id=" + formId + "]:last input:visible,form[id=" + formId
					+ "]:last textarea:visible,form[id=" + formId
					+ "]:last select:visible").each(function(i) {
		// $("form[id=" + formId +
		// "]:last").children("input:visible,textarea:visible,select:visible").each(function(i)
		// {
 ...
 * @returns {boolean}
 */
Validator.validFormDataType= function(formId) {
	var isValid = true;
	$(
		"form[id=" + formId + "]:last input:visible,form[id=" + formId
		+ "]:last textarea:visible,form[id=" + formId
		+ "]:last select:visible").each(function(i) {
		if (!Validator.validDataType($(this), true)) {
			isValid = false;
			return false;
		}
	});
...
Validator.onblur = function(jqueryObj) {
	jqueryObj.find("input:visible,textarea:visible,select:visible").unbind(
			'blur.validator').bind('blur.validator', function() {
...
// || str.indexOf('.', 0) == -1
// || str.lastIndexOf("@") == str.length - 1 || str.lastIndexOf(".") ==
// str.length - 1) {
// return false;
//
// } else
...
//function getPhoneType(phone) {
//	var lt_regexp = /^(86|1)(3[0-2]|5[56]|8[56]|4[5]|7[1]|7[6])\d{8}$|^1709\\d{7}$/;
//
//	var dx_regexp = /^(86|1)(3[3]|5[3]|7[3]|7[7]|8[01]|8[9])\d{8}$|^1700\\d{7}$/;
//
...
function isTwoDecimal(str){
	...
	if(/^[0-9]{1,20}\.[0-9]{1,2}$|^[0-9]{1,20}$/.test(str)){
		return true;
	}else
		return false;
}
...
function moneyWithFlot(str){// 验证最多三位小数
	if (str == '') {
		return true;
	}
...
function moneyWithTwo(str){// 验证最多两位小数
	if (str == '') {
		return true;
	}
...
/**
 * 判断是否是非法字符，即不含特殊字符 -除外
 * 
 */
function isNormal2(str) {
	if (str == '') {
		return true;
	}
	var reg = '~!@$%^&*+{}|:\'<>?`=[]/\\';
