...
layui.use(['element', 'laydate','table', 'form', 'layer'], function () {
    ...
    var active = {
        ...
        export: function () {
            var flag = checkExport('companyDetail_filepath');
            if(!flag){
            ...
            if (valid()) {
                ...
                if(optionsData !=null){
	                var cookieName = guid();
	                url = LOC_RES_BASE_PATH + '/rptCompanyDetail/exportCompanyDetailAjax?'
	                    ...
	                    + "&isFrameContract=" + optionsData.isFrameContract
    ...
    function getParam() {
        ...
        var isFrameContract = $("#isFrameContract").val();//是否签框架  yes/no
        ...
        optionsData.isFrameContract = isFrameContract;
    ...
    function reloadData(optionsData) {
        ...
        table.reload('contentReload', {
            url: BASE_PATH + '/rptCompanyDetail/queryCompanyDetailList',
            cols: setCols(),
    ...
    function setCols() {
        ...
        row1.push(
            ...
            {field: 'comCompanyNo', title: '公司编号', width: 150, align: 'center', colspan: 1,rowspan: 2},
            {field: 'companyName', title: '公司名称', width: 300, align: 'center', colspan: 1,rowspan: 2
            	, templet: function (d) {
            ...
            {field: 'isFrameContract', title: '是否签框架', width: 150, align: 'center', colspan: 1,rowspan: 2
            	,templet: function (d) {
                    var str = "";
                    if(d.isFrameContract =='yes'){
                        str = "是";
                    }else{
                    	str = "否";
                    }
            ...
            {field: 'frameDateStart', title: '框架开始时间', width: 150, align: 'center', colspan: 1,rowspan: 2
            	,templet: function (d) {
                    if (d.frameDateStart == null || d.frameDateStart == "") {
                        return "";
                    } else {
                        return formatDate(d.frameDateStart, "yyyy-MM-dd");
                    }
                }
            },
            {field: 'frameDateEnd', title: '框架结束时间', width: 150, align: 'center', colspan: 1,rowspan: 2
            	,templet: function (d) {
                    if (d.frameDateEnd == null || d.frameDateEnd == "") {
                        return "";
                    } else {
                        return formatDate(d.frameDateEnd, "yyyy-MM-dd");
                    }
                }
            },
            {field: 'frameApprovePassDate', title: '框架过审时间', width: 150, align: 'center', colspan: 1,rowspan: 2
            	,templet: function (d) {
                    if (d.frameApprovePassDate == null || d.frameApprovePassDate == "") {
                        return "";
                    } else {
                        return formatDate(d.frameApprovePassDate, "yyyy-MM-dd");
                    }
                }
            },
            {field: 'frameCreateUserName', title: '框架协议创建人', width: 150, align: 'center', colspan: 1,rowspan: 2},
            {field: 'storeNums', title: '旗下门店数', width: 150, align: 'center', colspan: 1,rowspan: 2}
        );
    ...
    function fileInterval() {
        var url = BASE_PATH + '/expendReport/fileIntervalByName/' + "companyDetail_filepath";
    ...
    function downLoadExcel2(fileId) {
        ...
        var url = BASE_PATH + '/expendReport/queryReportSize/' + "companyDetail_filepath";
        $.ajax({
            type: "GET",
            url: url,
            async: true,
