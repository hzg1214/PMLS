 ...
 */
initList = function() {
	httpGet('SceneEstateListForm', 'LoadCxt', BASE_PATH + '/sceneEstate/qSceneEstate', function() {

		pageInfo("SceneEstateListForm", function() {
			initList();
		});
	});
	initCityDist = false;
...
SceneEstate.setSearchParams = function (searchParamMap) {
	...
	$("#SceneEstateListForm").find("input").each(function () {
	...
	$("#SceneEstateListForm").find("select").each(function () {
		var searchKey = $(this).attr("id");
		var searchValue = jsonMap[searchKey];
		if (searchValue != undefined && searchValue != null && searchValue != '') {
			$("#" + searchKey).val(searchValue);
