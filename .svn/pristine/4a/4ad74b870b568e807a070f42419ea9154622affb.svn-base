...
import cn.com.eju.pmls.poi.ExcelForLinkDyDetailBigDataNew;
 ...
 * @date   :2019年7月18日
 */
@Service("pmlsLinkDyDetailService")
public class PmlsLinkDyDetailService extends BaseService<LinkDetailDto> {
    ...
    @Async("threadPoolTaskExecutorPmlsLinkDyDetail")
    public void downLoadExcelAjax(Map<String, Object> reqMap, String fileCode, String fileName, int id) throws Exception {
        try {
            ...
            //资金成本-垫佣
            instance = new ExcelForLinkDyDetailBigDataNew();
...
//            ((ExcelForLinkDetail)instance).downloadSheet(allList, new File(pathName), yearly);
            //((ExcelForLinkDyDetailBigDataNew) instance).downloadSheet(reqMap, new File(pathName), yearly, qcTimeStage, zljTimeStage, dnyqTimeStage, dnxzTimeStage);
            ((ExcelForLinkDyDetailBigDataNew) instance).downloadSheet(reqMap, new File(pathName),yearly);
            ...
            //减
        	pmlsReportQueueService.updateReportQueueTotal("linkZjcbDetail_filepath", 2);
        }
    }
}
