...
layui.use(['upload'], function () {
    ...
    upload.render({
        elem: '#selectFile'
        , url: BASE_PATH + '/remittanceTrack/imput/'
        , data: {
...
//                    layer.closeAll();
                    ...
                    layer.alert(data.returnMsg, {icon: 1, title: '提示'}, function (index) {
                    	if(!isBlank(dataParams.trackType)
                                && !isBlank(dataParams.year)
                                && !isBlank(dataParams.yearMonthVersion)
                                && !isBlank(dataParams.weekVersion)
                        ){
