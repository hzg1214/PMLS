...
layui.use(['element', 'laydate', 'form', 'table', 'layer'], function () {
    ...
    function setCols() {
        ...
        row1.push(
            ...
            {field: 'inComeName',title: '收入类型',width:115, align: 'center',rowspan: 2}
        );
        ...
        if(estateType=='24503'){
            row1.push(
                {field: '',title: '应计返佣小计',width:150, align: 'center',colspan: 2},
                {field: '',title: '应计返佣',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整1',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整2',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整3',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整4',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整5',width:150, align: 'center',colspan: 3},
                {field: '',title: '应计返佣调整6',width:150, align: 'center',colspan: 3}
            );
        }
        if(estateType=='24504'){
            row1.push(
        ...
        if(estateType=='24505'){
            row1.push(
                {field: '',title: '实际返佣小计',width:150, align: 'center',colspan: 2},
                {field: '',title: '实际返佣',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整1',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整2',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整3',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整4',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整5',width:150, align: 'center',colspan: 3},
                {field: '',title: '实际返佣调整6',width:150, align: 'center',colspan: 3}
            );
        }
    ...
    active = {
        ...
        export: function () {
            if (valid()) {
                ...
                var url = BASE_PATH + '/pmlsSceneCommission/exportCheck?cityNo=' + $("#cityNo").val() + "&projectDepartmentId=" + $("#projectDepartmentId").val()
                    + "&searchKey=" + encodeURIComponent(trimStr($("#searchKey").val())) + "&reportId=" + trimStr($("#reportId").val()) + "&countDateStart=" +$("#countDateStart").val()
                    + "&countDateEnd=" + $("#countDateEnd").val() + "&estateType=" + $("#estateType").val()
                    + "&estateNmKey=" + getXMSelectIds(formSelects.value('estateNmKey'))+"&businessType=" + $("#businessType").val();
                $.ajax({
                    type: "GET",
                    url: url,
                    dataType:"json",
                    ...
                    success: function (data) {
                        if (data.returnCode == 200 || data.returnCode == '200') {
                            window.location.href = BASE_PATH + '/pmlsSceneCommission/export?cityNo=' + $("#cityNo").val() + "&projectDepartmentId=" + $("#projectDepartmentId").val()
                                + "&searchKey=" + encodeURIComponent(trimStr($("#searchKey").val())) + "&reportId=" + trimStr($("#reportId").val()) + "&countDateStart=" +$("#countDateStart").val()
                                + "&countDateEnd=" + $("#countDateEnd").val() + "&estateType=" + $("#estateType").val()
                                + "&estateNmKey=" + getXMSelectIds(formSelects.value('estateNmKey'))+"&businessType=" + $("#businessType").val();
        ...
        import: function () {
            ...
            layer.open({
                ...
                title: 'EXCEL导入',
                area: ['500px', '400px'],
                content: BASE_PATH + '/pmlsSceneCommission/toView'
            });
    ...
    function reloadData(optionsData) {
        ...
        table.reload('contentReload', {
            url: BASE_PATH + '/pmlsSceneCommission/qSceneLinkCommission',
            cols: setCols(),
            where: optionsData,
            page:{
                curr: 1 //重新从第 1 页开始
