...
initList = function() {
	...
	httpGet('StatisticCompanyListForm', 'LoadCxt', BASE_PATH + '/statistic/qCompany', function() {

		pageInfo("StatisticCompanyListForm", function() {
			initList();
		});
 ...
 * 
 */
StatisticCompany.search = function() {
	$('#curPage').val('1');
	initList();
};
...
//填充储存的表单值
StatisticCompany.setSearchParams = function (searchParamMap) {
	var jsonMap = JSON.parse(searchParamMap);
	
	$("#StatisticCompanyListForm").find("input").each(function () {
	...
	$("#StatisticCompanyListForm").find("select").each(function () {
		var searchKey = $(this).attr("id");
		var searchValue = jsonMap[searchKey];
		if (searchValue != undefined && searchValue != null && searchValue != '') {
			$("#" + searchKey).val(searchValue);
