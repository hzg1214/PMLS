...
<script type="application/javascript">
	var lnkYjPlanDtoJson = '${lnkYjPlanDtoJson}';
	var planId='${planId}';
	console.log(planId);
	console.log(lnkYjPlanDtoJson);
</script>
<body>
<div>
	<div class="layui-card">
		<div class="layui-card-body" style="min-height:500px;height: auto;">
			<div class="layui-form yjPlanForm">
				<input type="hidden" name="id" id="id" value="${lnkYjPlanDto.id}">
                <input type="hidden" id="projectNo" name="projectNo" value="${lnkYjPlanDto.projectNo}"/>
				<div class="layui-form-item">
					<label class="layui-form-label"><span class="redSpan">*</span>方案类型</label>
					<div class="layui-input-block">
						<input type="radio" id="planType0" name="planType" value="28701" title="收入" <c:if test="${lnkYjPlanDto.planType eq '28701'}">checked</c:if> lay-filter="planType">
						<input type="radio" id="planType1" name="planType" value="28702" title="返佣" <c:if test="${lnkYjPlanDto.planType eq '28702'}">checked</c:if> lay-filter="planType">
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" id="planName" name="planName" style="width: 337px;"  value="${lnkYjPlanDto.planName}" lay-verify="required" maxlength="10" placeholder="请输入不超过10个汉字" autocomplete="off"  class="layui-input">
					</div>
				</div>
				...
				<div class="layui-form-item" id="fyPlanTypeDiv" style="display: none">
					...
					<div class="layui-input-block">
						<input type="radio" name="fyPlanType" value="28801" title="标准" <c:if test="${lnkYjPlanDto.fyPlanType eq '28801'}">checked</c:if> lay-filter="fyPlanType">
						<input type="radio" name="fyPlanType" value="28802" title="特殊" <c:if test="${lnkYjPlanDto.fyPlanType eq '28802'}">checked</c:if> lay-filter="fyPlanType">
					</div>
				</div>
					...
					<div class="layui-input-block">
						<div class="layui-input-inline" style="width: 100px;">
							<select id="periodType" name="periodType" lay-verify="required" lay-search="" lay-filter="periodType">
								...
								<option value="28901" <c:if test="${lnkYjPlanDto.periodType eq '28901'}">selected</c:if> >报备</option>
								<option value="28902" <c:if test="${lnkYjPlanDto.periodType eq '28902'}">selected</c:if> >带看</option>
								<option value="28903" <c:if test="${lnkYjPlanDto.periodType eq '28903'}">selected</c:if> >大定</option>
								<option value="28904" <c:if test="${lnkYjPlanDto.periodType eq '28904'}">selected</c:if> >成销</option>
							</select>
						</div>
				...
				<div class="layui-form-item" style="margin-bottom: 5px;">
					...
					<div class="layui-input-block">
						<input type="radio" name="equationType" value="28101" title="每套"  <c:if test="${lnkYjPlanDto.equationType eq '28101'}">checked</c:if>	lay-filter="equationType">
						<input type="radio" name="equationType" value="28102" title="每平米" <c:if test="${lnkYjPlanDto.equationType eq '28102'}">checked</c:if> lay-filter="equationType">
						<input type="radio" name="equationType" value="28103" title="每套总价" <c:if test="${lnkYjPlanDto.equationType eq '28103'}">checked</c:if>	lay-filter="equationType">
						<input type="radio" name="equationType" value="28104" title="每套总面积（㎡）" <c:if test="${lnkYjPlanDto.equationType eq '28104'}">checked</c:if> lay-filter="equationType">
					</div>
				</div>
...
<script src="${pageContext.request.contextPath}/meta/pmls/js/yjPlan/yjPlanEdit.js?v=${vs}"></script>
</body>
</html>
