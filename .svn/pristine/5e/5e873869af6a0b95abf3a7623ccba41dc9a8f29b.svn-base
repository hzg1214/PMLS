...
public class ExcelForLinkDyDetailBigDataNew extends ExcelUseXmlAbstract {
    private final LogHelper logger = LogHelper.getLogger(this.getClass());

    private PmlsLinkDyDetailService linkZjcbDetailService = new PmlsLinkDyDetailService();
...
//        this.dnxzTimeStage = dnxzTimeStage;
    ...
    private void createSheet(Writer out, Map<String, Object> reqMap) throws Exception {
        ...
        logger.info("联动资金成本(垫佣资金)第1次查询开始。。。");
        ...
        if (contentList != null && totalCount > contentList.size()) {
            ...
            for (int i = 2; i <= pageCount; i++) {
                ...
                logger.info("联动资金成本(垫佣资金)第" + i + "次查询开始,共" + pageCount + "次。");
                reback = linkZjcbDetailService.queryLinkZjcbDetailList(reqMap, pageInfo);
                logger.info("联动资金成本(垫佣资金)第" + i + "次查询结束,共" + pageCount + "次。");
                ...
                logger.info("联动资金成本(垫佣资金)第" + i + "次写文件结束。");
            }
        }
        logger.info("联动资金成本(垫佣资金)查询结束！");
    ...
    private int createSheetHead(SpeedSheetWriter sw, int index, List<String> listMerge) throws IOException {
        ...
        if (Integer.parseInt(yearly) > 2019) {
            nowColIndex = createBlock(sw, "2019年", 3, 1, listMerge);
        }
        ...
        if (Integer.parseInt(yearly) > 2019) {
            nowColIndex = createBlock(sw, "累计垫佣", 1, 1, listMerge);
            nowColIndex = createBlock(sw, "累计回款", 1, 1, listMerge);
            nowColIndex = createBlock(sw, "累计资金成本", 1, 1, listMerge);
        }
        nowColIndex = createBlock(sw, "总累计" , 1, 1, listMerge);
        nowColIndex = createBlock(sw, "当年以前" , 1, 1, listMerge);
        nowColIndex = createBlock(sw, "当年新增" , 1, 1, listMerge);

        if (Integer.parseInt(yearly) > 2019) {
            for (int i = 0; i < 12; i++) {
                nowColIndex = createBlock(sw, "垫佣金额", 1, 1, listMerge);
    ...
    private int createSheetBody(SpeedSheetWriter sw, int index, LinkedHashMap<String, Object> map) throws Exception {
        ...
        if (Integer.parseInt(yearly) > 2019) {
            sw.createCell(i++, clearZero(map.get("dy2019beflj")), doubleBlack);
            sw.createCell(i++, clearZero(map.get("hk2019beflj")), doubleBlack);
            sw.createCell(i++, clearZero(map.get("zjcb2019beflj")), doubleBlack);
        }
