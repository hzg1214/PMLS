...
import cn.com.eju.deal.dto.houseLinkage.estate.*;
 ...
 * Service层
 */
@Service("pmlsEstateService")
public class PmlsEstateService extends BaseService<EstateInfoDto> {
    ...
    public ResultData<?> update(Map<String, Object> reqMap, EstatePhotosDto estatePhotosDto)
        ...
        estateInfoDto.getEstate().setProjectStatus(DictionaryConstants.Project_Status_Start);
        ...
        estateInfoDto.setOperationType("isUpt");
...
//    }
    ...
    private EstateInfoDto SetNewEstateInfoDto(Map<String, Object> reqMap, EstatePhotosDto estatePhotosDto)
        ...
        String projectLeaderEmpId = (String) reqMap.get("projectLeaderEmpId");//项目负责人userId
        ...
        // 预计当季大定金额
        String subscribedThisQuarterStr = (String) reqMap.get("subscribedThisQuarter");
        if (StringUtil.isNotEmpty(subscribedThisQuarterStr)) {
            BigDecimal subscribedThisQuarter = new BigDecimal(subscribedThisQuarterStr);
            estateDto.setSubscribedThisQuarter(subscribedThisQuarter);
        }
        // 预计第一季度大定金额
        String subscribedQuarter1Str = (String) reqMap.get("subscribedQuarter1");
        if (StringUtil.isNotEmpty(subscribedQuarter1Str)) {
            BigDecimal subscribedQuarter1 = new BigDecimal(subscribedQuarter1Str);
            estateDto.setSubscribedQuarter1(subscribedQuarter1);
        }
        // 预计第二季度大定金额
        String subscribedQuarter2Str = (String) reqMap.get("subscribedQuarter2");
        if (StringUtil.isNotEmpty(subscribedQuarter2Str)) {
            BigDecimal subscribedQuarter2 = new BigDecimal(subscribedQuarter2Str);
            estateDto.setSubscribedQuarter2(subscribedQuarter2);
        }
        // 预计第三季度大定金额
        String subscribedQuarter3Str = (String) reqMap.get("subscribedQuarter3");
        if (StringUtil.isNotEmpty(subscribedQuarter3Str)) {
            BigDecimal subscribedQuarter3 = new BigDecimal(subscribedQuarter3Str);
            estateDto.setSubscribedQuarter3(subscribedQuarter3);
        }
        // 预计第四季度大定金额
        String subscribedQuarter4Str = (String) reqMap.get("subscribedQuarter4");
        if (StringUtil.isNotEmpty(subscribedQuarter4Str)) {
            BigDecimal subscribedQuarter4 = new BigDecimal(subscribedQuarter4Str);
            estateDto.setSubscribedQuarter4(subscribedQuarter4);
        }
        // 预计当年大定金额
        ...
        String empTel = (String) reqMap.get("empTel");   //开发负责人电话
        String projectLeaderTel = (String) reqMap.get("projectLeaderTel");   //项目负责人电话
        ...
        if (reqMap.containsKey("opEstateId")) {
            estateDto.setOpEstateId(reqMap.get("opEstateId").toString());
        }
        if (reqMap.containsKey("opEstateNm")) {
        ...
        //开发商ID
        if (reqMap.containsKey("devCompanyId")) {
            estateDto.setDevCompanyId(reqMap.get("devCompanyId").toString());
        }
        //开发商名称
        if (reqMap.containsKey("devCompanyText")) {
            estateDto.setDevCompany(reqMap.get("devCompanyText").toString());
        }
        //刷筹
        if (reqMap.containsKey("brushRaiseFlag")) {
            if (StringUtil.isNotEmpty(reqMap.get("brushRaiseFlag").toString())) {
                Integer brushRaiseFlag = Integer.valueOf(reqMap.get("brushRaiseFlag").toString());
                estateDto.setBrushRaiseFlag(brushRaiseFlag);
            }
        }
        //共场
        if (reqMap.containsKey("totalFieldFlag")) {
            if (StringUtil.isNotEmpty(reqMap.get("totalFieldFlag").toString())) {
                Integer totalFieldFlag = Integer.valueOf(reqMap.get("totalFieldFlag").toString());
                estateDto.setTotalFieldFlag(totalFieldFlag);
            }
        }
        //是否借佣
        if (reqMap.containsKey("excuseCommisionFlag")) {
            if (StringUtil.isNotEmpty(reqMap.get("excuseCommisionFlag").toString())) {
                Integer excuseCommisionFlag = Integer.valueOf(reqMap.get("excuseCommisionFlag").toString());
                estateDto.setIsExcuseCommision(excuseCommisionFlag);
            }
        }
        estateDto.setDeveloperName((String) reqMap.get("developerName"));//开发商全称

        estateDto.setEmpTel(empTel);
        estateDto.setProjectLeaderTel(projectLeaderTel);
        ...
        if (StringUtil.isNotEmpty(bigCustomerFlag)) {
            ...
            if ("22601".equals(bigCustomerFlag)) {
                if (reqMap.containsKey("bigCustomerId")) {
        ...
        String partnerAbbreviationNm 	= (String) reqMap.get("partnerAbbreviationNm");
        String partnerAbbreviationCode 	= (String) reqMap.get("partnerAbbreviationCode");
        String developerBrandId 			= (String) reqMap.get("developerBrandId");
        String developerBrandName 		= (String) reqMap.get("developerBrandName");


        estateDto.setPartnerAbbreviationNm(partnerAbbreviationNm.trim());
        estateDto.setPartnerAbbreviationCode(partnerAbbreviationCode.trim());
        if(StringUtil.isNotEmpty(developerBrandId)){
            estateDto.setDeveloperBrandId(Integer.parseInt(developerBrandId));
        }
        estateDto.setDeveloperBrandName(developerBrandName.trim());
        ...
        if (null != projectLeaderEmpId && StringUtil.isNotEmpty(projectLeaderEmpId)) {
        	estateDto.setProjectLeaderEmpId(Integer.valueOf(projectLeaderEmpId));
        }
        ...
        //是否垫佣甲方
        Integer custodyFlgStr = null;
        if(reqMap.get("custodyFlg")!=null && StringUtil.isNotEmpty(reqMap.get("custodyFlg").toString())){
            custodyFlgStr = Integer.valueOf(reqMap.get("custodyFlg").toString());
        }
        estateDto.setCustodyFlg(custodyFlgStr);
        if (custodyFlgStr!=null && custodyFlgStr == 1) {//是   垫佣甲方简称
            if (reqMap.containsKey("mattressNailId")) {
                String mattressNailIdStr = reqMap.get("mattressNailId").toString();
                Integer mattressNailId = Integer.valueOf(mattressNailIdStr);
                estateDto.setMattressNailId(mattressNailId);//垫佣甲方简称
            }
        }


        if(reqMap.get("vendibilityAmount")!=null && StringUtil.isNotEmpty(reqMap.get("vendibilityAmount").toString())){
            BigDecimal vendibilityAmount = new BigDecimal(reqMap.get("vendibilityAmount").toString());
            BigDecimal baseAccount = new BigDecimal("100000000");
            vendibilityAmount = vendibilityAmount.setScale(2, BigDecimal.ROUND_HALF_UP).multiply(baseAccount);
            estateDto.setVendibilityAmount(vendibilityAmount);
        }
        if(reqMap.get("monthRoughAmount")!=null && StringUtil.isNotEmpty(reqMap.get("monthRoughAmount").toString())){
            BigDecimal monthRoughAmount = new BigDecimal(reqMap.get("monthRoughAmount").toString());
            BigDecimal baseAccount = new BigDecimal("100000000");
            monthRoughAmount = monthRoughAmount.setScale(2, BigDecimal.ROUND_HALF_UP).multiply(baseAccount);
            estateDto.setMonthRoughAmount(monthRoughAmount);
        }
        if(reqMap.get("cooperationStatus")!=null){
            estateDto.setCooperationStatus(reqMap.get("cooperationStatus").toString());
        }
        if(reqMap.get("projectInfoDesc")!=null){
            estateDto.setProjectInfoDesc(reqMap.get("projectInfoDesc").toString());
        }
        if(reqMap.get("requiredForSupport")!=null){
            estateDto.setRequiredForSupport(reqMap.get("requiredForSupport").toString());
        }
        estateInfoDto.setEstate(estateDto);
        //联动规则
    ...
    private EstateInfoDto SetEditEstateInfoDto(Map<String, Object> reqMap, EstatePhotosDto estatePhotosDto)
        ...
        //开发商ID
        if (StringUtil.isNotEmpty(reqMap.get("devCompanyId").toString())) {
            estateDto.setDevCompanyId(reqMap.get("devCompanyId").toString());
        }
        //开发商名称
        if (StringUtil.isNotEmpty(reqMap.get("devCompanyText").toString())) {
            estateDto.setDevCompany(reqMap.get("devCompanyText").toString());
        }
        //刷筹
        if (reqMap.containsKey("brushRaiseFlag")) {
...
//        		estateDto.setDevCompany(devCompany);
        ...
        estateDto.setPromotionName(promotionName);
        estateDto.setSignName(signName);
        estateDto.setCountryNo((String) reqMap.get("countryNo"));
        estateDto.setOpEstateId((String) reqMap.get("opEstateId"));
        estateDto.setOpEstateNm((String) reqMap.get("opEstateNm"));
...
//        List<PhotoDto> filePhoto = this.getPhotos(reqMap, estateId, typeId);
    ...
    public ResultData<?> updatePopupDetail(Map<String, Object> reqMap) throws Exception {
        ...
        String oldEstatePosition = (String) reqMap.get("oldEstatePosition");
        String oldAddress = (String) reqMap.get("oldAddress");
        String newCountryNo = (String) reqMap.get("newCountryNo");
        String newCountryNm = (String) reqMap.get("newCountryNm");
        ...
        if (null != estateAddress) {
            ...
            if ("0".equals(oldEstatePosition)) {
                ...
                estateDto.setCountryNo(newCountryNo);
                addDetail = "原项目地址:" + oldAddress + ";<br>修改后项目地址:" + newCountryNm + newAddress;
            }
    ...
    public ResultData<?> getProgrammeList(Map<String, Object> reqMap, PageInfo pageInfo) throws Exception {
        String url = SystemParam.getWebConfigValue("RestServer") + "estate" + "/getProgrammeList/{param}";
        ResultData<?> reback = get(url, reqMap, pageInfo);
        return reback;
    }

    public ResultData<?> queryQueryYJAmount(Map<String, Object> reqMap) throws Exception {
        String url = SystemParam.getWebConfigValue("RestServer") + "lnkYjPlan" + "/queryQueryYJAmount/{param}";
        ResultData<?> reback = get(url, reqMap);
        return reback;
    }
    public ResultData<?> getLastProjectByCity(Map<String, Object> param)  throws Exception {
        String url = SystemParam.getWebConfigValue("RestServer") + "estate" + "/lastProject/{param}";
        ResultData<?> backResult = get(url, param);
        return backResult;
    }
    
    public ResultData<?> getProjectLeaderSelList(Map<String, Object> reqMap, PageInfo pageInfo) throws Exception {
        //调用 接口
        String url = SystemParam.getWebConfigValue("RestServer") + "estate" + "/getProjectLeaderSelList/{param}";
        ResultData<?> reback = get(url, reqMap, pageInfo);
        return reback;
    }
     ...
     * @return Estate
     */
    public ResultData<?> getEstateById(int id)
            throws Exception {
        //调用 接口
        String url = SystemParam.getWebConfigValue("RestServer") + "estate" + "/getEstateById/{id}/";
        ResultData<?> backResult = get(url, id);
        return backResult;
    }
     ...
     * @throws Exception
     */
    public ResultData<?> uptCooperationStatus(Integer id,String cooperationStatus)
            throws Exception {
        String url = SystemParam.getWebConfigValue("RestServer") + "estate/uptCooperationStatus";

        EstateInfoDto estateInfoDto = new EstateInfoDto();
        EstateDto estateDto = new EstateDto();
        estateDto.setId(id);
        estateDto.setCooperationStatus(cooperationStatus);
        estateInfoDto.setEstate(estateDto);

        ArrayList<EstateChangeDto> ecList = new ArrayList<>();
        UserInfo userInfo = UserInfoHolder.get();
        EstateChangeDto estateChangeDto = new EstateChangeDto();
        estateChangeDto.setChangeType(String.valueOf(DictionaryConstants.Project_Change_CooperationStatus));
        estateChangeDto.setChangeName("项目合作意向状态变更");
        if("1".equals(cooperationStatus)){
            estateChangeDto.setChangeDetail("合作意向状态:由一般跟单变更为极有意向");
        }else {
            estateChangeDto.setChangeDetail("合作意向状态:由极有意向变更为一般跟单");
        }
        estateChangeDto.setChangeDate(new Date());
        estateChangeDto.setEstateId(id);
        estateChangeDto.setOperator(userInfo.getUserId());
