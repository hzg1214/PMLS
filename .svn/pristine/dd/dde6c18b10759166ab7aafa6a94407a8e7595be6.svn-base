...
layui.use(['element', 'laydate', 'form', 'table', 'layer'], function () {
    ...
    function reloadData() {
        ...
        var companyType = $("#companyType").val(); //1只要大渠道 2既要分销渠道也要大渠道
        var companyNo = trimStr($("#companyNo").val());
        var companyName = trimStr($("#companyName").val());
        var parentCompanyNo = trimStr($("#parentCompanyNo").val());
        optionsData.companyNo = companyNo;
        optionsData.companyName = companyName;
        optionsData.companyType = companyType;
        optionsData.parentCompanyNo = parentCompanyNo;

        table.reload('mainTable', {
            url: BASE_PATH + '/borrowMoneyFrameContract/getBorrowMoneyFrameContractCompanyList',
            cols: setCols(),
