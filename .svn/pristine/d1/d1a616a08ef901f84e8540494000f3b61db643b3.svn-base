...
public class ExcelForLinkProjectPartATraceBigData extends ExcelUseXmlAbstract {
    ...
    private PmlsLinkProjectPartATraceService pmlsLinkProjectPartATraceService = new PmlsLinkProjectPartATraceService();

    public void downloadSheet(Map<String, Object> reqMap, File file) throws IOException {
        OutputStream os = new FileOutputStream(file);

        XSSFWorkbook wb = new XSSFWorkbook();
        XSSFSheet sheet = wb.createSheet("联动项目甲方跟踪");
    ...
    private void createSheet(Writer out, Map<String, Object> reqMap) throws Exception {
        ...
        ResultData<?> reback = pmlsLinkProjectPartATraceService.queryLinkProjectPartATraceList(reqMap, pageInfo);
        ...
        logger.info("第一次查询联动项目甲方跟踪，totalCount:" + totalCount + ",时间：" + DateUtils.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
        ...
        if (totalCount > contentList.size()) {
            ...
            for (int i = 2; i <= pageCount; i++) {
                ...
                logger.info("第" + i + "次查询联动项目甲方跟踪,共" + pageCount + "次。时间：" + DateUtils.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
        ...
        logger.info("联动项目甲方跟踪查询完成，时间：" + DateUtils.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
    ...
    private int createSheetHead(SpeedSheetWriter sw, int index, List<String> listMerge) throws IOException {
        ...
        nowColIndex = createBlock(sw, "甲方简称", 1, 3,headerStyle, listMerge);
        ...
        nowColIndex = createBlock(sw, "总累计", 24, 1, listMerge);
        ...
        nowColIndex = createBlock(sw, "应计坏账(自行计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应计坏账(抵房坏账)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应计坏账(诉讼计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应计坏账(惩罚性计提)", 1, 2, listMerge);

        nowColIndex = createBlock(sw, "应收坏账(自行计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应收坏账(抵房坏账)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应收坏账(诉讼计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "应收坏账(惩罚性计提)", 1, 2, listMerge);

        nowColIndex = createBlock(sw, "开票坏账(自行计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "开票坏账(抵房坏账)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "开票坏账(诉讼计提)", 1, 2, listMerge);
        nowColIndex = createBlock(sw, "开票坏账(惩罚性计提)", 1, 2, listMerge);
    ...
    private int createSheetBody(SpeedSheetWriter sw, int index, LinkedHashMap<String, Object> map) throws Exception {
        ...
        sw.createCell(i++, map.get("partnerAbbreviationNm") == null ? "" : map.get("partnerAbbreviationNm").toString(), bodyLeftStyle);
        ...
        sw.createCell(i++, clearZero(map.get("totalYjBadDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYjBadDfDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYjBadssDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYjBadPunishDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYsBadDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYsBadDfDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYsBadssDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalYsBadPunishDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalKpBadDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalKpBadDfDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalKpBadssDebt")), moneyStyle);
        sw.createCell(i++, clearZero(map.get("totalKpBadPunishDebt")), moneyStyle);
    ...
    public List<ExcelColumnSet> setColumnWidth() {
        ...
        //开票账龄
        c = new ExcelColumnSet(String.valueOf(nColIndex), String.valueOf(nColIndex + 21));
        c.setWidth(18);
        columnSet.add(c);
        nColIndex += 21;
