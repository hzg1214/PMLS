	...
	$(".ui_state_highlight").click(function(){
		...
		if (Validator.validForm("companyEditForm")) {
			...
			if($("#chb3").is(':checked')== true) {
				...
				//var len = $("#newBusinessLicenseNo").val().length;
				if(len!=15 && len!=18) {
					$("#busWarning").html("营业执照必须是15位或18位！");
					$("#busWarning").show();
					return false;
				}
				if($("#newBusinessLicenseNo").val().trim()==$("#businessLicenseNo").val().trim())
				{
					$("#busWarning").html("修改后营业执照号和原营业执照号一样！");
					$("#busWarning").show();
					return false;
				}
			}
...
function changeContractUlChange(obj){
    if($("#changeContractUl").size()==0 && $("#changeContractUl2").size()==0 && $("#changeContractUl3").size()==0){
        return;
    }
